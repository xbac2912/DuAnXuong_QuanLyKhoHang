...
//        String tb_khohang=  "CREATE TABLE tb_khohang (MaSp TEXT, tenSP TEXT, GiaSp INTEGER, soLuong INTEGER, tenLoai TEXT)";
        db.execSQL(tb_User);
        db.execSQL(tb_loai);
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if(newVersion != oldVersion) {
            ...
            db.execSQL("Drop table if exists tb_loaihang");
            db.execSQL("Drop table if exists tb_SanPham");
            db.execSQL("Drop table if exists tb_khohang");
            db.execSQL("Drop table if exists tb_phieuxuat");
            db.execSQL("Drop table if exists tb_phieunhap");
            db.execSQL("Drop table if exists tb_CTPhieuxuat");
            onCreate(db);
        }
    }
}
